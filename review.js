...
async function loadAndRenderCards(containerSelector) {
    ...
    const targetGrid = document.querySelector(containerSelector);
    if (!targetGrid) {
        console.error(`Grid container not found for selector: ${containerSelector}`);
        return; 
    }

    targetGrid.innerHTML = arr.map(item => {
...
loadAndRenderCards('#ru .grid').then(() => {
    initializeSortingCourse('#ru'); 
    initializeSortingDate('#ru');   
});

loadAndRenderCards('#exp .grid').then(() => {
